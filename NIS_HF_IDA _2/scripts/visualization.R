...
mortality <- ggplot(df, aes(x = HF, y = Percentage, fill = HF)) +
  ...
  theme(
    ...
    axis.line.x = element_line(color = "black", size = 0.6),  # keep x axis line
    axis.line.y = element_line(color = "black", size = 0.6),  # keep y axis line

    axis.text.x = element_text(vjust = -1, hjust = 0.5, size = 16),
    axis.text.y = element_text(vjust = 0, hjust = 1, size = 16),
    axis.ticks = element_line(),
    axis.title.x = element_blank(),
    axis.title.y = element_text(vjust = 8, hjust = 0.5, size = 18, face = "bold"),
  ...
  scale_fill_manual(values = c("HFpEF" = "#b34745", "HFrEF" = "#0172b6"))

ggsave("Mortality.tiff", mortality, width = 6, height = 6, units = "in", bg = "white")
...
LOS <- ggplot(df, aes(HF, Value, fill = HF)) +
  ...
  geom_text(aes(label = round(Value, 1)),
            vjust = 2, colour = "white", size = 6, fontface = "bold") +
  theme_ipsum(base_size = 16) +
...
ggsave("LOS.tiff", LOS, width = 6, height = 6, units = "in", bg = "white")
...
ggsave("Charge.tiff", Charge, width = 6, height = 6, compression = "lzw" units = "in", bg = "white")
