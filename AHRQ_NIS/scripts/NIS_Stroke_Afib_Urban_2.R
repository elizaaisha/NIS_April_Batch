...
dta_clean <- dta_base |>
  mutate(
    HOSP_LOCTEACH = case_when(
      HOSP_LOCTEACH %in% c("Urban, non-teaching", "Urban, teaching") ~ "Urban",
      .default = HOSP_LOCTEACH
    ),
    Stroke = case_when(
    ...
    PL_NCHS = case_when(
      PL_NCHS %in% c("Central metro ≥1 million", "Fringe metro ≥1 million",
                     "Metro 250,000-999,999", "Metro 50,000-249,999") ~ "Urban",
      PL_NCHS %in% c("Micropolitan", "Other")                         ~ "Rural",
      .default = PL_NCHS   # keep original value for everything else
    ),
    ...
    Thrombectomy = case_when(str_detect(PR10_Combined, "03CG\\d*") ~ "Yes", .default = "No"),
    ...
    Mechanical_Ventilation = case_when(str_detect(PR10_Combined, "5A09B5K|5A09C5K|5A09D5K|5A1935Z|5A1945Z|5A1955Z") ~ "Yes", .default = "No"),
...
arrow::write_dataset(
  dta_clean,
  "./nis_data/NIS_Stroke_Afib_Urban_2.parquet",
  format = "parquet",
  partitioning = "YEAR"
)
