...
-   **Study Objective**: Examine the impact of aspiration pneumonia on in-hospital outcomes among inpatient admissions with a Parkinson's disease.

-   **Data Source**: Cross-sectional analysis of the National Inpatient Sample (NIS) from 2018 to 2020.

-   **Patient Selection**: Included all inpatient admissions with a Parkinson's disease.

-   **Aspirtaion Pneumonia + Parkinson's Disease Categories**:

    -   Parkinson's Disease with Aspirtaion Pneumonia

    -   Parkinson's Disease without Aspirtaion Pneumonia
...
### Discharged to short-term facility:
...
### Discharged to long-term facility:
...
reg_model_LOS <- svyglm(
  LOS ~ PD_AP + AGE + FEMALE + RACE + elixsum + ZIPINC_QRTL + Insurance + Hosp_Census_Region +
    HTN + DM + Cancer + CHF + Hyperlip + CAD + PVD + CKD + COPD + Cardiac_arrhythmia + Dementia + Liver_Dis + Fluid_disorders,
  design = dsgn,
  family = "gaussian"
)
...
reg_model_TOTCHG <- svyglm(
  adj_TOTCHG ~ PD_AP + AGE + FEMALE + RACE + elixsum + ZIPINC_QRTL + Insurance + Hosp_Census_Region + HTN + DM + Cancer + CHF + Hyperlip + CAD + PVD + CKD + COPD + Cardiac_arrhythmia + Dementia + Liver_Dis + Fluid_disorders,
  design = dsgn,
  family = "gaussian"
  )
